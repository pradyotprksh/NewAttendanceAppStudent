...
/**
 * Created by pradyotprakash on 27/02/18.
 */
...
public class FingerprintHandler extends FingerprintManager.AuthenticationCallback {
    ...
    private void update(String s, boolean b) {
        TextView paraLabel = ((Activity) context).findViewById(R.id.paragraphLable);
        ImageView fingerprintImage = ((Activity) context).findViewById(R.id.fingerprintImage);
        paraLabel.setText(s);
        if (b == false) {
            paraLabel.setTextColor(ContextCompat.getColor(context, R.color.colorAccent));
        } else {
            paraLabel.setTextColor(ContextCompat.getColor(context, R.color.colorPrimaryDark));
            fingerprintImage.setImageResource(R.mipmap.ic_fingerprint_done);
            sendToMain();
        }
    }
